...
def feature_add(
    ...
    # Use a transaction to ensure atomicity when creating the feature
    try:
        with TransactionManager(repos):
            # 1. Create Feature using the repositories
            feature = Feature(
                name=name,
            ...
            # 2. Perform any other necessary initial setup here (e.g., default story or dependency creation)
...
def feature_update(
    ...
    # We rely on the Model's standard save/update for simple field updates,
    # as the TransactionManager is primarily for creation/related writes.
    if name is not None:
        feature.name = name
...
def feature_assign(
    ...
    try:
        ...
        console.print(f"[red]Error assigning feature: {e}[/red]")
        raise typer.Exit(1)
    feature = Feature(
        name=name,
        description=description,
